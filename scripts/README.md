...
| `build-evolution-expert-release-artifacts.mjs` | `npm run evolution-expert:release:artifact` | Builds the independently versioned Evolution Expert npm Candidate, SPDX SBOM, provenance, and checksums without publishing. |
| `verify-evolution-expert-release-artifacts.mjs` | `npm run verify:evolution-expert-release-artifact` | Verifies the exact Expert package identity, dependency binding, SBOM, provenance, checksums, and closed release-set contents. |
...
```bash
npm run ecs:immutable-rollout -- --version 2.2.0 --host root@8.153.72.80 --apply --json
```
