...
const contract = JSON.parse(fs.readFileSync(path.resolve("governance/acceptance/v61-completion-contract.json"), "utf8"));
...
if (evidencePath) {
  ...
  if (validator.installedArtifactRequired && raw.installationVerified !== true) throw new Error("CRITERION_INSTALLED_ARTIFACT_REQUIRED");
  ...
  const request = {
    ...
    requiredCandidateVersions: { runtime: "6.1.0", expert: "2.1.0" },
    evidenceTemplate: {
      ...
      candidateDigests: { runtime: "sha256:<runtime-6.1.0-candidate>", expert: "sha256:<expert-2.1.0-candidate>" },
