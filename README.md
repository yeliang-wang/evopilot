...
[![Release](https://img.shields.io/badge/GA%20Release-v1.1.1-2ea043)](#release-status)
...
| Self-host now | You want the API, worker, code-upgrader, Postgres, and Dashboard together | `bash -c "$(curl -fsSL https://raw.githubusercontent.com/yeliang-wang/evopilot/v1.1.1/install.sh)"` |
...
The latest published GitHub release is **v1.1.1 GA**, a code-structure evolution release over the original `v1.0.0` GA baseline.

v1.1.1 keeps the v1.0.10 distribution surface, moves the HTTP control-plane runtime behind an explicit runtime boundary, keeps `server.ts` as a thin compatibility adapter, extracts HTTP, runtime-auth, executor-adapter, and storage helpers, and verifies those boundaries during `npm run check`.

Release evidence:

- Latest release notes: [docs/releases/1.1.1.md](docs/releases/1.1.1.md)
