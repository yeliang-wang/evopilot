...
| 项目接入 | 通过 GitHub、GitLab 或本地 Git 向导注册项目，验证源码凭据、CI/CD 和部署连接器；同时提供 Project Workspace 和 Connector Marketplace 设置视图。 |
| 发现与目标 | 统一展示 Discovery Runtime、Target Runtime、Target Backlog、证据策略、评测集和机会点。 |
| Loop 执行 | 创建 source-to-production loop，查看 Source-to-GA 动态本体链路图、Visual Loop Run Canvas、Workflow Canvas Editor、Interactive Run Console、worker queue、runtime 表格、trace、replay 和 sandbox 证据。 |
...
Loop 执行页的 Source-to-GA 动态本体链路图不是静态流程图。它把当前选中的 LoopRun 与项目、Discovery、Target Backlog、ExecutorGraph、worker/sandbox、human gate、`sourceClosure`、deploy finalizer、`sourceReleaseRun` 和 release decision 合成一条可读链路：`SCM/Git Project -> Discovery Candidate -> Target Backlog -> Executor Graph -> Worker + Sandbox -> Human Gate -> Source Closure -> CI/CD + Deploy -> Release Decision -> GA Release`。相邻的 Workflow Canvas Editor 展示创建或调整 Loop 时的业务本体：`Target -> Discovery -> Executor -> Evaluator -> Human gate -> Release`。两张图共同回答三个问题：这个目标从哪个源码/证据进入 Loop，当前卡在哪个受控执行边界，离 GA 所需的 release decision、artifacts、policy 和 health-ready 证据还缺什么。
