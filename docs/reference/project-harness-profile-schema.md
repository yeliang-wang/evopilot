...
The v2 domain model keeps product domain, compatibility, architecture, and implementation runtime separate. For example, `database-product-harness@2.2.0` is for the owner's self-developed database product; PostgreSQL and MySQL are compatibility references or differential oracles, not the default evolution target.
