...
| Code structure | Thin server package entrypoint and compatibility adapter, thin HTTP composition root, focused route modules, application helper boundary, file-backed storage boundary, runtime auth/config helpers, executor adapter module, release target/scenario/risk helper module, HTTP helpers, server contracts, harness-template domain defaults, thin CLI process entrypoint, command runtime module, and architecture-boundary verification. | `packages/server/src/index.ts`, `packages/server/src/server.ts`, `packages/server/src/runtime/control-plane-runtime.ts`, `packages/server/src/application/control-plane-services.ts`, `packages/server/src/http/`, `packages/server/src/storage/file-store/`, `packages/cli/src/index.ts`, `packages/cli/src/commands/runtime.ts`, `scripts/verify-architecture-boundaries.mjs` |
...
| Code structure | Entry, server compatibility adapter, Dashboard shell, HTTP helpers, route modules, storage primitives, `FileStore` boundary, application helper boundary, runtime auth/config, executor adapters, release target helpers, thin control-plane runtime, thin CLI entrypoint, and command runtime are bounded and guarded | Continue splitting the large transitional application helper and CLI command runtime into domain modules as those areas change. |
