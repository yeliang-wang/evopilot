...
| `harness-catalog.read` / `harness-catalog.scan.warning` | 定位配置的 Published Harness Catalog 目录、catalogId、catalogDigest、entryDigest、warnings 和 nextAction。 |
| `goal-plan.selected-harness-bound` / `goal-plan.selected-harness-missing` | 定位目标规划是否从当前 Catalog 中选择了 published Harness。 |
