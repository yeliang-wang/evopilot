...
| Self-host now | New operators bringing up a complete stack | `bash -c "$(curl -fsSL https://raw.githubusercontent.com/yeliang-wang/evopilot/v2.5.0/install.sh)"` |
...
```powershell
iwr https://raw.githubusercontent.com/yeliang-wang/evopilot/v2.5.0/install.ps1 -OutFile install.ps1
...
```bash
curl -fsSL https://raw.githubusercontent.com/yeliang-wang/evopilot/v2.5.0/install.sh \
  | EVOPILOT_INSTALL_PACKAGE_SPEC=create-evopilot@2.5.0 bash -s -- --dir evopilot-stack
cd evopilot-stack
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/yeliang-wang/evopilot/v2.5.0/install.sh | bash -s -- --dir evopilot-stack --start
```
...
```bash
npm run verify:npm-registry -- --version 2.5.0
```
