...
| Self-host now | New operators bringing up a complete stack | `bash -c "$(curl -fsSL https://raw.githubusercontent.com/yeliang-wang/evopilot/v1.1.6/install.sh)"` |
...
```bash
curl -fsSL https://raw.githubusercontent.com/yeliang-wang/evopilot/v1.1.6/install.sh \
  | EVOPILOT_INSTALL_PACKAGE_SPEC=create-evopilot@1.1.6 bash -s -- --dir evopilot-stack
cd evopilot-stack
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/yeliang-wang/evopilot/v1.1.6/install.sh | bash -s -- --dir evopilot-stack --start
```
