...
The tagged installer resolves `create-evopilot` from the release manifest. In v1.1.7, the default package spec is the GitHub Release tarball because public npm registry packages are not published yet.
...
```bash
npx create-evopilot@1.1.7 self-host --dir evopilot-stack --init-env
```
