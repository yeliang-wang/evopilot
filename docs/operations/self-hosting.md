...
The tagged installer resolves `create-evopilot` from the release manifest. In v2.4.2, the default package spec is the GitHub Release tarball because public npm registry packages are not published yet.
...
```bash
npx create-evopilot@2.4.2 self-host --dir evopilot-stack --init-env
```
