...
The public npm registry package remains a separate post-publish layer. Use `npm install -g @evopilot/cli@2.4.2` only after `npm run verify:npm-registry -- --version 2.4.2` passes.
...
Template harness maintenance is an administrator workflow, not a daily project execution step. Fresh installs include runtime templates `python-enterprise-harness`, `java-ddd-service-harness`, `node-saas-control-plane-harness`, `go-middleware-harness`, and `observability-apm-harness`, plus v2.2 domain templates `database-product-harness` and `api-gateway-harness`. Project onboarding automatically matches one published template from product-domain signals, project context, and the goal loop target; `--from-template` is only an explicit administrator or advanced override. Domain templates are `@2.2.0`; existing runtime and broad templates remain `@1.1.0` baselines and are not part of the current database/gateway domain upgrade. Use `harness template inspect <id> --json` to read a published template's `sourceReferences[]`, capabilities, runtime defaults, validation baseline, `domainExecution`, evidence contract, failure taxonomy, diagnostics, observability, governance, phase mapping, and changelog. For source-driven template evolution, the ordinary entry is `harness evolve --source-project <path> --goal <text> --json`; it auto-matches, creates or resumes a `HarnessTemplateEvolution`, advances to `REVIEW_REQUIRED`, and returns draft/sourceCoverage/validation/diff/nextAction without approving or publishing. The atomic lifecycle commands remain available for administrators and automation: `harness template match`, `harness template evolution create|advance|approve|publish|impact`. For human editing, use the directory packs under `harness-templates/public/<template-id>/` and the small admin CLI surface: `harness template pack list`, `harness template pack validate`, and `harness template pack publish`. Direct `harness template upgrade --file <template.yaml> --changelog <text> --json` remains available for YAML/JSON publishing. Reusing the same `id@version` requires `--force`; existing active project profiles keep their templateRef until a reviewed profile revision is generated or upgraded and activated.
