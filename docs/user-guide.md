...
下一轮建议聚焦的 6 个 target 已内置进 Target Loop Backlog，用户可以像操作其他 target 一样推进：`discovery-skill-runtime` 用于把仓库、trace、评测和生产信号发现沉淀成可复用技能；`per-finding-worktree-handoff` 用于给每个 finding 分配隔离 worktree、分支、文件范围和验证命令；`adversarial-evaluator-agent` 用于在 merge/deploy 前独立挑战 diff、测试和发布证据；`recurring-loop-scheduler` 用于按时间窗、证据阈值和预算触发周期性 loop；`loop-memory-inbox` 用于把历史发现、用户反馈、失败评测和发布经验整理成可转 target 的 inbox；`budget-and-judgment-guardrails` 用于把成本、token、时间、影响面、置信度和发布判断变成显式 stop condition。
