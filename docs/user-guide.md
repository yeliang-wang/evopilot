...
- `ecs-docker-compose`：EvoPilot 在配置的服务器 `workingDir` 内执行受限发布序列，先读取当前 commit，可选执行 `git pull --ff-only <remote> <branch>`，再执行 `docker compose -f <composeFile> up -d --build [serviceName]`，并把每条命令的退出码和截断输出写入 deploy gate 证据。
