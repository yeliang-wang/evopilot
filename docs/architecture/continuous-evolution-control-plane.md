...
| `HarnessTemplate` profile | Platform or tenant baseline inherited by project profiles. Administrators publish versioned templates with changelog through `POST /api/v1/harness/templates` or `evopilot harness template apply`; template upgrades create DRAFT profile revisions and do not silently rewrite active project profiles. |
