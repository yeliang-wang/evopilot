...
| `HarnessTemplate` profile | Public platform or administrator-published language/software-type baseline inherited by project profiles. EvoPilot automatically matches a published template during project profile generation unless an administrator explicitly overrides it. Administrators publish versioned templates with changelog through `POST /api/v1/harness/templates` or `evopilot harness template upgrade`; template upgrades create DRAFT profile revisions and do not silently rewrite active project profiles. |
...
| Project harness control | `HarnessTemplate`, `TenantHarnessPolicy`, `ProjectHarnessProfile` versions, validation, diff, activation, explain mapping, and goal-plan profile/policy digest binding |
