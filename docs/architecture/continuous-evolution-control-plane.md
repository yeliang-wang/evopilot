...
For Harness-aware planning, `evopilot-harness` publishes the definitions, `CATALOG.md`, and `harness-registry.yaml`. EvoPilot reads the configured Registry, follows enabled Catalog roots, selects one published Harness for the project and objective, and binds that selection into `GoalPlan.selectedHarness`. If no suitable Harness exists, EvoPilot records the missing selection and the operator should publish or improve the Harness in `evopilot-harness`, then generate a new plan.
