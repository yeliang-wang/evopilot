...
`packages/server/src/index.ts` is a thin package entrypoint, and `packages/server/src/server.ts` is a thin compatibility adapter for public exports and direct start. `packages/server/src/runtime/control-plane-runtime.ts` owns the current HTTP composition wiring during migration: it wires route modules, authentication handoff, request logging, storage, and application helpers into the domain through explicit ports. File-backed persistence lives in `packages/server/src/storage/file-store/`, while transitional control-plane use-case helpers live in `packages/server/src/application/control-plane-services.ts`. Runtime mode resolution, production auth/config checks, user/session token handling, authorization, RBAC helpers, and audit record construction live in `packages/server/src/runtime/runtime-auth.ts`; loop executor adapter registration and step evidence assembly live in `packages/server/src/runtime/executor-adapters.ts`. Shared server contracts live in `packages/server/src/model.ts`; HTTP errors, request logging, structured server logging, response writers, static Dashboard serving, platform readiness/version builders, route registry, low-coupling route groups, and file-storage primitives live under `packages/server/src/http/` and `packages/server/src/storage/` so the package entrypoint and compatibility adapter do not own reusable interface concerns.
