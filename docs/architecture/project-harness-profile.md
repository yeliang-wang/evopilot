...
When template changes come from reviewable sources, the administrator uses `HarnessTemplateEvolution`. It collects source snapshots from `web-url`, `github-repo`, `gitlab-repo`, `source-project`, `source-corpus`, `production-log`, `evopilot-history`, `attachment`, `local-pack`, `runtime-evidence`, `existing-template`, or `admin-note` sources, analyzes signals, generates a DRAFT template pack, requires explicit approval, publishes a new `HarnessTemplate` version, and reports affected project profiles. This adds a lifecycle around template changes without letting LLM output bypass validation or approval. HarnessTemplate upgrades are normal control-plane template versions; an EvoPilot binary release is only required when the extractor, schema, API, or Dashboard code itself changes.
