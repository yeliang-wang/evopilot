...
Fresh installs include runtime `HarnessTemplate` baselines for Python enterprise projects, Java DDD services, Node SaaS control planes, Go middleware, and observability/APM systems, plus v2 domain templates for database products and API gateways. Domain templates are `@2.2.0`; runtime and broad templates remain `@1.1.0` baselines. Project onboarding automatically matches one published template from domain signals, runtime/repository context, and the goal loop target; an explicit template id is an administrator or advanced override.
...
When template changes come from reviewable sources, the administrator uses `HarnessTemplateEvolution`. It collects source snapshots from `web-url`, `github-repo`, `gitlab-repo`, `source-project`, `source-corpus`, `production-log`, `evopilot-history`, `attachment`, `local-pack`, `runtime-evidence`, `existing-template`, or `admin-note` sources, analyzes signals, generates a DRAFT template pack, requires explicit approval, publishes a new `HarnessTemplate` version, and reports affected project profiles. `harness template match` and `evolution create --auto-match` add a server-side decision layer before the lifecycle starts: EvoPilot can evolve an existing domain template or create a new target such as `distributed-cache-harness@0.1.0` from a runtime base such as `go-middleware-harness@1.1.0`. This adds a lifecycle around template changes without letting LLM output bypass validation or approval. HarnessTemplate upgrades are normal control-plane template versions; an EvoPilot binary release is only required when the extractor, matcher, schema, API, or Dashboard code itself changes.
