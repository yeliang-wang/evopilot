...
The backlog now carries the next six GA-alignment targets as product runtime objects rather than a separate roadmap note:

- `discovery-skill-runtime`: a context-layer discovery contract that turns repository, trace, evaluation, production, and manual signals into target candidates with provenance.
- `per-finding-worktree-handoff`: a harness-layer handoff contract that allocates isolated worktrees, branches, allowed paths, validation commands, and resumable executor context per finding.
- `adversarial-evaluator-agent`: a harness-layer evaluator that independently challenges proposed diffs, tests, release evidence, and completion claims before merge or deployment.
- `recurring-loop-scheduler`: a loop-layer scheduler for cadence, evidence-threshold, release-window, and budget-driven recurring loops.
- `loop-memory-inbox`: a context-layer inbox for prior findings, failed evaluations, feedback, release learnings, and operator notes that can be triaged into target loops.
- `budget-and-judgment-guardrails`: a loop-layer guardrail target that makes cost, token, time, blast-radius, confidence, and release-judgment thresholds explicit stop conditions.
