...
async function assertDashboardContract(baseUrl) {
  ...
  for (const navLabel of ["工作台", "项目接入", "发现与目标", "Loop 执行", "评估与发布", "历史审计"]) {
    assert.match(app, new RegExp(navLabel));
  }
  for (const label of ["工作台", "产品闭环", "治理", "Autopilot cockpit", "一键自动驾驶从项目到生产发布", "启动一键自动驾驶", "人工待办中心", "待补凭据", "待批准", "待修复", "待发布", "项目接入向导", "Project workspace", "Overview", "Targets", "Runs", "Credentials", "Deployments", "连接器市场与设置", "GitHub", "GitLab", "Local Git", "Jenkins", "ECS / K8s / Webhook", "LLM Route", "Visual Loop Run Canvas", "Executor graph", "Workflow Canvas Editor", "Graph template", "条件路由", "从画布创建 Loop", "Interactive run console", "实时 Agent 运行控制台", "读取 Streaming Events", "当前 executor", "Release cockpit", "源码到生产发布检查清单", "安全自动合并", "进化观测图", "项目拓扑", "运行证据", "已接入", "OpenTelemetry", "SkyWalking", "用户反馈", "触发来源", "触发时间", "IP", "证据摘要", "评测集", "Eval Dataset", "Regression Suite", "形成机会点", "关联评测集", "查看方案", "编辑进化方案", "Markdown 方案正文", "提交方案修改", "确认进化", "Target Runtime", "Discovery Runtime", "运行 Discovery", "创建每日 Target Schedule", "/api/v1/loop-target-runtime/discovery/run", "/api/v1/loop-target-runtime/schedules", "/api/v1/loop-target-runtime/summary", "Loop Runtime", "闭环编排", "创建闭环 Loop", "Target Loop Backlog", "Codex target loop", "推进下一 Target", "一键自动驾驶", "/api/v1/loop-orchestration/advance", "/api/v1/loop-orchestration/autopilot", "Worker Queue Workbench", "Claim 下一 Loop", "评估与发布门禁", "运行独立评估", "评估预算门禁", "/api/v1/loop-target-runtime/adversarial-evaluations", "/api/v1/loop-target-runtime/guardrails", "Release Run Auto Repair Workbench", "/api/v1/source-release-runs/repair-candidates", "一键修复队列", "Deploy Finalizer Workbench", "/api/v1/source-release-deploy-finalizers", "/api/v1/loop-workers/claim", "Context Time Travel Workbench", "Replay 并生成 Diff", "/time-travel/replay", "Sandbox Boundary Workbench", "验证 Sandbox Proof", "/sandbox-proof/verify", "Streaming Trace Workbench", "刷新 Trace Tree", "/trace-tree", "/events", "Source Closure Workbench", "Release Closure Runtime", "刷新 Release Run", "批准 Release", "合并 Release", "安全自动合并", "修复 Release Run", "repair-source-release-run", "Policy", "Post Merge Deploy", "/source-closure/plan", "/source-closure/review-decision", "/api/v1/source-release-runs", "source-release-closure-runtime", "Worker", "/api/v1/loops", "代码升级过程", "根据方案进行代码升级", "白盒执行", "查看原始执行事件", "execution-transcript", "CI/CD 阶段视图", "代码升级失败", "历史详情", "注册项目", "验证并注册", "Git URL", "Token 环境变量", "使用系统默认 Jenkins", "使用项目独立 Jenkins", "Jenkins Job", "/api/v1/evaluation-datasets", "/api/v1/opportunity-drafts", "/api/v1/code-upgrade-runs", "/code-upgrade"]) {
