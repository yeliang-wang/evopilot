...
2. Expose structured tool results, local or remote MCP, and exact human-decision presentation.
...
A conformant adapter binds one Core digest, one protocol version, one adapter
digest, and a compatible Runtime range. It preserves Runtime-owned state,
