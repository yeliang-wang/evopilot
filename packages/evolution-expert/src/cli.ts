...
import { EVOLUTION_EXPERT_CORE, EVOPILOT_EVOLUTION_EXPERT_VERSION, createExpertAdapter, expertCompatibility, expertDoctor, expertTutorial, planExpertTurn, renderInteraction } from "./index.js";
...
if (command === "--version" || command === "version") output = { product: "@evopilot/evolution-expert", version: EVOPILOT_EVOLUTION_EXPERT_VERSION };
  ...
  output = expertCompatibility(adapter, args[2] ?? "5.0.1", ["structured-tool-results", "local-or-remote-mcp", "human-decision-presentation"]);
} else if (command === "doctor") output = expertDoctor(args[1] ?? "generic-agent", args[2] ?? "5.0.1", ["structured-tool-results", "local-or-remote-mcp", "human-decision-presentation"]);
