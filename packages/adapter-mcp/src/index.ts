...
export const EVOPILOT_LIFECYCLE_MCP_TOOLS: EvoPilotLifecycleMcpTool[] = [
  ...
  { name: "evopilot_lifecycle_observation_record", description: "Record exact Runtime, project, Lifecycle, HarnessBundle, GoalTarget, Host, Provider, environment, authority, evaluator, scorer, and evidence-bound Pipeline observations.", method: "POST", path: "/api/v1/controlled-lifecycle/observations", authority: "NONE" },
  { name: "evopilot_lifecycle_observation_inspect", description: "Inspect one immutable Pipeline observation and its deterministic gap classification.", method: "GET", path: "/api/v1/controlled-lifecycle/observations/{observationId}", authority: "NONE" },
  { name: "evopilot_lifecycle_gap_classify", description: "Classify a Pipeline observation as a declarative successor, governed-resource successor, generic Runtime primitive, Expert guidance gap, or no action without mutating active state.", method: "POST", path: "/api/v1/controlled-lifecycle/observations/{observationId}/classify", authority: "NONE" },
  { name: "evopilot_lifecycle_successor_propose", description: "Create an immutable Lifecycle successor proposal with semantic, authority, dependency, compatibility, impact, migration, experiment, monitoring, rollback, and provenance diffs.", method: "POST", path: "/api/v1/controlled-lifecycle/successors", authority: "NONE" },
  { name: "evopilot_lifecycle_successor_inspect", description: "Inspect one exact immutable Lifecycle successor proposal without activating it.", method: "GET", path: "/api/v1/controlled-lifecycle/successors/{proposalId}", authority: "NONE" },
  { name: "evopilot_lifecycle_experiment_evaluate", description: "Evaluate Champion and Challenger only under an identical governed context and stratify any mismatch instead of mixing evidence.", method: "POST", path: "/api/v1/controlled-lifecycle/experiments", authority: "NONE" },
  { name: "evopilot_lifecycle_activation_decide", description: "Apply an exact active safe-activation policy; auto-activate future runs only when every preauthorized predicate passes, otherwise return a bound human decision or rejection.", method: "POST", path: "/api/v1/controlled-lifecycle/activation-decisions", authority: "NONE" },
  { name: "evopilot_lifecycle_monitoring_evaluate", description: "Evaluate declared post-activation health signals and perform receipt-safe deterministic rollback without rebinding active runs or replaying uncertain mutations.", method: "POST", path: "/api/v1/controlled-lifecycle/monitoring/evaluate", authority: "NONE" },
  { name: "evopilot_generic_primitive_target_propose", description: "Create a digest-bound Runtime or Expert Target proposal for a missing project-neutral primitive and block project-specific Core branching.", method: "POST", path: "/api/v1/controlled-lifecycle/target-proposals", authority: "NONE" },
